...
func TestFormUrl(t *testing.T) {
	...
	client := weather.NewClient(testApiKey)
...
func TestParseJson(t *testing.T) {
	...
	// E.G. want := &weather.ApiResponse{List:[{Weather:[{Description:clear sky}]}]}
	// but I can't quite get that syntax correct, so...:
	//
	// Construct each layer of `want`.
	want := weather.ApiResponse{}
	want.List = make([]weather.ApiResponseList, 1)
	want.List[0].Weather = make([]weather.ApiResponseListWeather, 1)
